"""
Complete fix for gemini.py - replace the entire break_story_into_scenes function
"""
...
for i, line in enumerate(lines):
    if 'async def break_story_into_scenes(' in line:
        start_idx = i
        break
...
# Find the end of the function (next async def)
end_idx = None
for i in range(start_idx + 1, len(lines)):
    if lines[i].strip().startswith('async def ') and not lines[i].strip().startswith('async def break_story'):
        end_idx = i
        break
...
corrected_function = '''    async def break_story_into_scenes(
        self, 
        story_text: str, 
...
new_lines = lines[:start_idx] + [corrected_function + '\n\n'] + lines[end_idx:]
...
print("✅ Successfully replaced break_story_into_scenes function")
